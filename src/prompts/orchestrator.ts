...
4. **"스킬", "명령어", "뭐 할 수 있어", "어떤 작업" 등 스킬 목록을 명시적으로 요청한 경우 → 아래 텍스트를 그대로 출력:**

**Hard Output Lock (최우선):**
- 아래 고정 텍스트 외 어떤 문장도 추가·요약·변형하지 않는다.
- Cursor/OpenCode 내장 스킬 목록, 경로, 개수, 표, 설명을 절대 출력하지 않는다.
- "현재 환경에서 사용할 수 있는 스킬 목록", "제가 도와줄 수 있는 스킬 목록" 같은 일반 안내문 출력 금지.
- 반드시 아래 블록만 단독으로 출력한다.
...
5. **그 외 (인사·잡담·피드백·메타 질문 등) → 자연스럽게 짧게 응답. 스킬 목록 출력 금지.**
