...
| 의도/키워드 | 스킬 | 트리거 예시 |
|------------|------|------------|
| 버그·오류·안됨 | \`debug\` | "왜 안돼?", "에러 고쳐줘", "TypeError 남" |
| 신규 기능·추가·구현·개발 | \`dev\` | "만들어줘", "추가해줘", "개발해줘" |
| 구조 개선·정리·리팩터 | \`refactor\` | "리팩터해줘", "코드 정리해줘" |
| 테스트·스펙·커버리지 | \`test\` | "테스트 짜줘", "커버리지 올려줘" |
| 코드 검토·리뷰 | \`review\` | "봐줘", "어때?", "검토해줘" |
| PR·풀리퀘·diff | \`review-pr\` | "PR 리뷰해줘", "이 diff 봐줘" |
| 요구사항·설계·계획 | \`plan\` | "어떻게 할지", "계획 세워줘", "설계해줘" |
| 보안·취약점·감사 | \`audit\` | "보안 검토해줘", "취약점 찾아줘" |
| 문서·README·주석 | \`doc\` | "문서화해줘", "README 써줘" |
| 배포·릴리즈 | \`release\` | "배포해도 돼?", "나가도 돼?" |
| 프로젝트 파악·초기화 | \`setup\` | "프로젝트 파악해줘", "셋업해줘" |
| MCP·연동 상태 | \`mcp\` | "MCP 확인해줘", "연동 봐줘" |

### 분류 규칙
1. 위 테이블로 매핑 가능하면 → 해당 스킬 절차 즉시 실행
2. 첫 메시지이고 의도를 전혀 파악할 수 없을 때 → 아래 스킬 목록을 그대로 출력:
...
3. 사용자가 스킬 목록을 명시적으로 요청할 때 ("뭐 할 수 있어?", "스킬 목록", "commands" 등) → 위 목록 출력
4. 대화 중 의도가 모호한 경우 → 짧게 되묻기만 ("어떤 작업인가요? 개발·리뷰·디버그 중 가장 가까운 것은?")
...
**Phase 3: 리포트**
\`✅ .agent-core/context.md 생성 완료 — 이후 매 턴 자동 주입\`

Anti-patterns: 실제 수집 없이 작성 금지 / 범위 선택 전 수집 시작 금지
