...
| `/core-setup` | 프로젝트 컨텍스트 수집 · 외부파일 등록 · 문서 템플릿 설정 · 저장 항목 관리 |
...
저장된 항목은 `/core-setup` → 모드 4로 조회·삭제할 수 있습니다.
...
| 범주 | 항목 |
|------|------|
| 구조 | 제목 형식, 섹션 순서, 출력 형식(.md/.docx/.pdf 등), 언어 |
| Markdown 디자인 | 섹션 구분선, 콜아웃 스타일(GitHub/blockquote), 코드블록 태깅, 다이어그램(Mermaid/PlantUML) |
| 리치 포맷 디자인 | 컬러 테마(HEX), 폰트(제목/본문), 표지, 로고·워터마크 |
| 공통 | 헤더·푸터 |

> 시작 시 "참고할 파일 경로?"를 물어봅니다. 기존 문서를 경로로 입력하면 구조·스타일을 자동 분석해 항목별 기본값을 제안합니다.

> 템플릿을 등록해두면 `/core-doc` 실행 시 목록에서 선택해 해당 구조·디자인으로 문서를 자동 생성합니다.
